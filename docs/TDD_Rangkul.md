...
1. SMS darurat.
2. Inbox realtime penuh; pertahankan chat per task sederhana.
3. Filter pengawasan RW detail dan dashboard komisi detail.
4. Badge otomatis; timeline dan grafik Riwayat Rangkul tetap dipertahankan.
...
## Amendment Mengikat Sprint 3 Farros, 27 Agustus 2026

Bagian ini mengikat keputusan Sprint 3 ketika ada referensi historis yang masih menyebut provider atau jalur demo lama. Struktur dan catatan historis sebelumnya tetap dipertahankan.

- Provider pembayaran Sprint 3 adalah Midtrans Sandbox. Demo Ledger, `saldo_demo`, `charge-dummy`, dan top-up Admin ditunda ke sprint berikutnya dan tidak menjadi acceptance criteria Sprint 3.
- Jalur pembayaran Sprint 3 hanya memakai checkout, webhook settlement, refund, kompensasi pembatalan, dan auto-release yang state-nya ditentukan server melalui RPC. Browser tidak menjadi sumber nominal, split, atau status final.
- Endpoint pembatalan canonical adalah `PATCH /api/tasks/:id/cancel`. Alias `POST` boleh dipertahankan untuk kompatibilitas client lama, tetapi tidak menjadi contract baru.
- Settlement wajib mencocokkan signature, order ID, status transaksi, `gross_amount`, dan snapshot `payments.amount` serta `payments.jumlah_total`. Retry tidak boleh menggandakan transaction log final atau saldo.
- Release normal memakai 90% Helper, 7% Platform, dan 3% Koordinator. Kompensasi pembatalan memakai 50% Helper dan 50% refund Keluarga tanpa fee Platform atau Koordinator.
- Dua laporan aktif mengubah Helper menjadi `under_review`. Status tersebut memblokir penerimaan task baru. Koordinator hanya dapat mereview Helper dalam wilayahnya, Admin dapat mereview seluruh wilayah, dan keputusan dengan perubahan status Helper wajib memiliki alasan serta audit log.
