...
Current verified source checks after this update: `npm run lint` exits 0 with 61 non-blocking warnings, `npm run typecheck` passes, `npm run test` reports 229 passed and 9 cloud-runtime skips because credentials are unavailable, `git diff --check` reports no whitespace error, and `next build` compiled, generated a fresh `.next/BUILD_ID`, then exited. `npm ci` could not complete on this Windows host after the workspace Next server locked `lightningcss`; after the exact workspace servers were stopped, npm stalled with no further CPU progress and was terminated. Re-run `npm ci` in a clean runner before release. The user opted out of Impeccable for this iteration, so no Impeccable result is counted. Four-viewport, keyboard, zoom, reduced-motion, and cloud RLS evidence remain mandatory before acceptance.
