...
| U5 Admin                 | Implemented, not accepted                | The isolated admin layout is replaced with the shared operational shell. A moderation-first action grid, quieter platform metrics, and human-readable audit activity are implemented. Source contracts, targeted lint/typecheck, and Impeccable detector pass. Full visual/state acceptance evidence remains.                                        |
| U6 Landing               | Implemented, not accepted                | Story now follows problem, visit, service, Riwayat Rangkul, trust controls, role entry, and final CTA. Four-viewport and keyboard visual evidence remain.                                                                                                                                                                                                 |
...
Current verified source checks after this update: `npm run lint`, `npm run typecheck`, `npm run test` (223 passed, 9 cloud-runtime tests skipped because credentials are unavailable), `npm run build`, and `git diff --check` pass after the current edits. `npm ci` could not complete on this Windows host after the workspace Next server locked `lightningcss`; after the exact workspace servers were stopped, npm stalled with no further CPU progress and was terminated. Re-run `npm ci` in a clean runner before release. Impeccable detector reports no finding for Navbar, RoleSidebar, mobile bottom navigation, Koordinator dashboard, Admin dashboard, and the Helper dashboard. Four-viewport, keyboard, zoom, reduced-motion, and cloud RLS evidence remain mandatory before acceptance.
...
4. Refine the role section so Keluarga, Helper, Koordinator, and Admin each show their real product responsibility and one route-backed next step. Keep actions usable from 375px.
...
Use `ui-ux-pro-max` and `gpt-taste` before major UI work, then use Impeccable as the final visual/accessibility review.
