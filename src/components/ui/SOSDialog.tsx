...
export default function SOSDialog({ isOpen, onClose, userRole, taskId: initialTaskId }: SOSDialogProps) {
  ...
  useEffect(() => {
    ...
    if (userRole !== "helper" || initialTaskId) return;
    void fetch("/api/tasks/active", { cache: "no-store" })
      .then(async (response) => response.json().then((body: { task?: { id: string } | null }) => {
        if (response.ok && body.task?.id) setTaskId(body.task.id);
        else setError("Tidak ada tugas aktif untuk SOS.");
      }))
      .catch(() => setError("Tugas aktif tidak dapat dimuat."));
  }, [initialTaskId, isOpen, userRole]);

  useEffect(() => {
    if (!isOpen || countdown <= 0 || isAlerting || isSent) return;
    const timer = window.setTimeout(() => setCountdown((value) => value - 1), 1000);
    return () => window.clearTimeout(timer);
  }, [countdown, isAlerting, isOpen, isSent]);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        {isSent ? (
          ...
          <><div className="relative overflow-hidden bg-red-600 p-8 text-center"><AlertTriangle className="relative z-10 mx-auto h-20 w-20 text-white" /><h2 className="relative z-10 mt-4 text-3xl font-black text-white">DARURAT</h2></div><div className="space-y-6 p-8 text-center"><p className="font-medium text-slate-600">Sinyal darurat akan dikirim ke pihak yang terkait tugas aktif.</p><div className="my-4 text-6xl font-black tabular-nums text-red-600">{countdown}</div>{error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}<div className="space-y-3"><Button onClick={handleSendAlert} disabled={isAlerting || !taskId} className="h-14 w-full rounded-2xl bg-red-600 text-lg font-bold text-white">{isAlerting ? <Loader2 className="mx-auto h-6 w-6 animate-spin" /> : "Kirim Sekarang"}</Button><Button variant="outline" onClick={onClose} disabled={isAlerting} className="h-14 w-full rounded-2xl">Batalkan</Button></div></div></>
        )}
      </div>
    </div>
  );
