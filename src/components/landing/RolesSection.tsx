...
function EarningSimulator({ config }: { config: EarningConfig }) {
  ...
  return (
    <aside className="relative overflow-hidden rounded-[26px] border border-blue-100 bg-white/90 p-4 shadow-[0_18px_40px_rgba(13,71,161,.14)] sm:p-5" aria-label={config.title}>
      ...
      <div className="mt-5 space-y-4">
        ...
        <fieldset>
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            {visitOptions.map((option) => <button key={option} type="button" aria-pressed={visits === option} onClick={() => setVisits(option)} className={`min-h-11 rounded-xl border px-3 text-sm font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${visits === option ? "border-primary bg-primary text-white shadow-[0_8px_16px_rgba(13,71,161,.2)]" : "border-blue-100 bg-white text-muted-foreground hover:border-primary/50 hover:text-primary"}`}>{option} kunjungan</button>)}
          </div>
        </fieldset>
      </div>

      <div className="mt-5 rounded-2xl bg-gradient-to-br from-slate-50 via-white to-blue-50 p-4 ring-1 ring-inset ring-blue-100">
        ...
        <div className="mt-3 flex items-end justify-between gap-3"><div><p className="text-xs font-bold text-primary">{config.resultLabel}</p><AnimatePresence mode="wait" initial={false}><motion.p key={summary} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .24 }} className="mt-1 font-heading text-2xl font-extrabold tracking-[-.03em] text-foreground" aria-live="polite">{formatRupiah(result)}</motion.p></AnimatePresence></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-extrabold text-primary ring-1 ring-blue-100">{config.rateLabel}</span></div>
        <p className="mt-2 text-xs leading-5 text-muted-foreground">{config.rateLabel} {config.supportingLabel}. Harga tambahan belum termasuk.</p>
      </div>

      <div className="mt-4 flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50/70 p-3 text-xs leading-5 text-blue-950"><Sparkles className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden="true" /><p><strong>Simulasi, bukan jaminan.</strong> Nominal mengikuti kunjungan yang selesai dan pembayaran dirilis.</p></div>
      <div className="mt-4 flex items-center gap-2 text-xs font-semibold text-muted-foreground"><Clock3 className="size-4 text-primary" aria-hidden="true" />Pembayaran online diproses setelah status kunjungan selesai.</div>
    </aside>
  );
}
