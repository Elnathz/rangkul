...
import { ChevronLeft, Loader2, Zap, Calendar } from "lucide-react";
...
type Category = { id: string; nama: string; tingkat: string; harga_dasar: number; estimasi_durasi_menit: number; is_high_risk: boolean; jarak_min_km: number | null; jarak_max_km: number | null };

export default function BookingNewPage() {
  const router = useRouter();
  const [lansias, setLansias] = useState<Lansia[]>([]);
  const [categories, setCategories] = useState<Category[]>([]);
  const [mode, setMode] = useState<"langsung" | "cepat">("langsung");
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, mode_penugasan: "langsung", jadwal_waktu: new Date(form.jadwal_waktu).toISOString() }) });
  ...
  return (
    <main className="min-h-screen bg-[#F5F8FC] px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        {/* Mode Penugasan Tabs */}
        <div className="grid grid-cols-2 gap-3 rounded-2xl bg-slate-200/60 p-1.5">
          <button
            type="button"
            onClick={() => setMode("langsung")}
            className={`flex items-center justify-center gap-2 rounded-xl py-3 text-xs font-bold transition ${
              mode === "langsung" ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:text-slate-900"
            }`}
          >
            <Calendar className="h-4 w-4 text-[#0D47A1]" /> Booking Biasa
          </button>
          <button
            type="button"
            onClick={() => setMode("cepat")}
            className={`flex items-center justify-center gap-2 rounded-xl py-3 text-xs font-bold transition ${
              mode === "cepat" ? "bg-white text-amber-900 shadow-sm" : "text-slate-600 hover:text-slate-900"
            }`}
          >
            <Zap className="h-4 w-4 text-amber-500" /> Cari Cepat (15 Menit)
          </button>
        </div>
          ...
          <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <label className="block text-sm font-bold text-slate-800">
              ...
              <select required value={form.lansia_id} onChange={(event) => setForm({ ...form, lansia_id: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 p-3 font-normal">
                ...
                {lansias.map((item) => <option key={item.id} value={item.id}>{item.nama} · {item.alamat}</option>)}
              </select>
            </label>
            <label className="block text-sm font-bold text-slate-800">
              Kategori layanan
              <select required value={form.service_category_id} onChange={(event) => setForm({ ...form, service_category_id: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 p-3 font-normal">
                ...
                {categories.map((item) => <option key={item.id} value={item.id}>{item.nama} · {item.tingkat} · Rp {Number(item.harga_dasar).toLocaleString("id-ID")}</option>)}
              </select>
            </label>
            <label className="block text-sm font-bold text-slate-800">
              Jadwal
            ...
            <p className="rounded-xl bg-blue-50 p-3 text-xs leading-relaxed text-blue-900">
              Kategori berbasis jarak seperti Antar Obat wajib dibuat dari halaman Cari Helper agar Helper dan koordinat lansia dapat diverifikasi server.
            </p>
            <Button type="submit" disabled={saving || !form.lansia_id || !form.service_category_id} className="w-full bg-[#0D47A1]">
              {saving ? <Loader2 className="h-5 w-5 animate-spin" /> : "Buat permintaan"}
            </Button>
          </form>
        )}
      </div>
