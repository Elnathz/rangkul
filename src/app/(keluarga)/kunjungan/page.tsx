...
async function TaskCard({ task }: { task: TaskRow }) {
  ...
  const helperFotoUrl = helper?.foto_wajah_url ? await resolvePrivateFileUrl(helper.foto_wajah_url) : null;
    ...
    <Card className="overflow-hidden border-slate-100 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <CardContent className="p-5 sm:p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
         ...
         <div className="mt-4 flex items-center gap-3 rounded-xl border border-slate-100 bg-white p-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100 text-sm font-black text-[#0D47A1]">{helperFotoUrl ? <img src={helperFotoUrl} alt="" className="h-full w-full object-cover" /> : helperUsers?.full_name?.slice(0, 1) || "H"}</div><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wider text-slate-700">Helper</p><p className="truncate text-sm font-bold text-slate-950">{helperUsers?.full_name || "Belum ditugaskan"}</p></div></div>
      </CardContent>
