...
const updateSchema = z.object({ wilayah: z.string().min(3, 'Wilayah minimal 3 karakter') });
...
export async function PATCH(request: Request) {
  try {
    ...
    const { data: profile, error } = await supabase.from('koordinator_profiles').update({ wilayah: validation.data.wilayah, ...(current.wilayah !== validation.data.wilayah ? { status: 'pending_verification' } : {}), updated_at: new Date().toISOString() }).eq('id', current.id).select('id, wilayah, tingkat, status, saldo_komisi, diverifikasi_at, created_at, updated_at').single();
