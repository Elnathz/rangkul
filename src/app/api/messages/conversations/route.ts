...
export async function GET() {
  ...
  const { data: messages, error } = await supabase.from("messages").select("id, sender_id, receiver_id, task_id, message, created_at, read_at").or(["sender_id.eq." + user.id, "receiver_id.eq." + user.id].join(",")).not("task_id", "is", null).order("created_at", { ascending: false });
