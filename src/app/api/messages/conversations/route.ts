...
export async function GET() {
  ...
  const { data: messages, error } = await supabase.from("messages").select("id, sender_id, receiver_id, task_id, message, created_at, read_at").or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`).order("created_at", { ascending: false });
  ...
  for (const message of messages || []) {
    const key = `${message.task_id || "global"}:${message.sender_id === user.id ? message.receiver_id : message.sender_id}`;
