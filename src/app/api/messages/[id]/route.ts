...
export async function GET(request: Request, context: RouteContext) {
  ...
  const { data: task, error: taskError } = await supabase.from("tasks").select("id, keluarga_id, helper_id, helper_profile:helper_profiles(user_id)").eq("id", taskId).maybeSingle();
  if (taskError) return createApiError("server_error", taskError.message, 500);
  const helperProfile = task ? (Array.isArray(task.helper_profile) ? task.helper_profile[0] : task.helper_profile) : null;
  if (!task || (task.keluarga_id !== user.id && helperProfile?.user_id !== user.id)) return createApiError("forbidden", "Anda bukan peserta tugas ini", 403);
